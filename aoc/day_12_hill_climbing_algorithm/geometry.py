...
class Grid:
    ...
    def __getitem__(self, key: PointLike, /) -> int:
        return self._data[key[0]][key[1]]

    def __setitem__(self, key: PointLike, val: int, /) -> None:
    ...
    @classmethod
    def fill(cls, shape: ShapeLike, fill: int = -1) -> Self:
        data = [[fill] * shape[1] for _ in range(shape[0])]
        out = cls(data)
        return out

    @classmethod
    def zeros(cls, shape: ShapeLike) -> Self:
        return cls.fill(shape, 0)

    def ndenumerate(self) -> Generator[tuple[Point, int], None, None]:
        for y in self.border.y:
            for x in self.border.x:
